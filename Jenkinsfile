...

{
 ...
 sh "docker push geethika609/maven-web-application:${buildNumber}"
  ...
  sshagent(['SSH'])
  {
  sh "ssh -o StrictHostKeyChecking=no ubuntu@13.232.148.203 sudo docker rm -f mavenwebapplicationcontainer || true"
  sh  "ssh -o StrictHostKeyChecking=no ubuntu@13.232.148.203 sudo docker run -d -p 8080:8080 --name mavenwebapplicationcontainer geethika609/maven-web-application:${buildNumber}"
  }
  }
  /*
